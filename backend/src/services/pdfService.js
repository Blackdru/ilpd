...
class PDFService {
  ...
  // Compress PDF
  async compressPDF(file, quality = 0.7, outputName = 'compressed.pdf') {
    if (file.type !== 'application/pdf') {
      throw new Error('File must be a PDF');
    }
    ...
    const compressedBuffer = Buffer.from(await pdf.save({
    ...
    // If compression didn't reduce size, return original with warning
    if (compressedBuffer.length >= fileBuffer.length) {
      throw new Error('PDF is already optimized and cannot be compressed further');
    }
